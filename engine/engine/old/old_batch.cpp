...
ED::Batch::Batch (const ED::Batch & other) {
	(*this) = other;
}
...
ED::Batch::Batch (Engine::Shader * shader_base) :
	Batch (nullptr, shader_base) {
	
}

// TODO : remove pointer, only ref is needed
ED::Batch::Batch (Engine::Layout *&&layout, Engine::Shader * shader_base) :
	m_should_resend_ssbo (true),
	m_visible_mesh_count (0),
	m_vertex_count (0),
	m_index_count (0),
...
ED::Batch::Batch (Engine::Layout *&& layout, std::vector<Engine::Mesh *> & meshes, Engine::Shader* shader_base) :
	m_should_resend_ssbo (true),
	m_visible_mesh_count (0),
	m_vertex_count (0),
	m_index_count (0),
...
void ED::Batch::Expand (Engine::Mesh * target_mesh) {
...
void ED::Batch::addMesh (Engine::Mesh * mesh) {
...
uint32_t ED::Batch::GetDiffuse () const {
	return (m_diffuseMap);
}

uint32_t ED::Batch::GetAmbient () const {
	return (m_ambientMap);
}

Engine::Shader *ED::Batch::GetShader () const {
	return (m_shader);
}

void ED::Batch::syncCommands () const {
	size_t		index_offset;
	size_t		vertex_offset;
	size_t		instance_offset;
	intptr_t 	draw_commands_offset;

	ED::DrawCommand command;
	...
	for (const auto &[mesh, instance_count] : m_instance_counter) {
		...
		glNamedBufferSubData (
			...
			sizeof (ED::DrawCommand),
			(void *) &command
		);

		draw_commands_offset += sizeof (ED::DrawCommand);
	}
}

void ED::Batch::syncInstancesSSBO () const {
	intptr_t offset;
	...
	for (const auto &obj : m_loaded_objects) {
		...
		glNamedBufferSubData (
			m_instance_ssbo,
			offset,
			sizeof (ED::InstanceData),
			(void*) &obj->m_data
		);
		offset += sizeof (ED::InstanceData);
	}
}