...
Engine::GameObject *Engine::Scene::CreateObject (Mesh *mesh, Engine::InstanceData data, Shader *shader) {
	Engine::Batch *batch;
	GameObject *object;
...
Engine::Batch *Engine::Scene::GetBatch (Shader *shader) {
	auto pos = std::find_if (m_batches.begin (), m_batches.end (), [shader](Engine::Batch &batch) -> bool {
