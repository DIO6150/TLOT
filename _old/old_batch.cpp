...
Engine::Batch::Batch (Engine::Shader * shader_base) :
	Batch (nullptr, shader_base) {
	
}

// TODO : remove pointer, only ref is needed
Engine::Batch::Batch (Engine::Layout *&&layout, Engine::Shader * shader_base) :
	m_should_resend_ssbo (true),
	m_visible_mesh_count (0),
	m_vertex_count (0),
	m_index_count (0),
	...
	vbo_size	= 1;
	ebo_size	= 1;
	dibo_size	= MAX_INSTANCE_COUNT * sizeof (Engine::DrawCommand);
	...
	glNamedBufferData (m_instance_ssbo, MAX_INSTANCE_COUNT * sizeof (Engine::InstanceData), NULL, GL_DYNAMIC_DRAW);
...
Engine::Batch::Batch (Engine::Layout *&& layout, std::vector<Engine::Mesh *> & meshes, Engine::Shader* shader_base) :
	m_should_resend_ssbo (true),
	m_visible_mesh_count (0),
	m_vertex_count (0),
	m_index_count (0),
...
void Engine::Batch::UploadGameObject (Engine::GameObject * object) {
...
void Engine::Batch::Expand (Engine::Mesh * target_mesh) {
...
void Engine::Batch::addMesh (Engine::Mesh * mesh) {
...
void Engine::Batch::syncCommands () const {
	size_t		index_offset;
	size_t		vertex_offset;
	size_t		instance_offset;
	intptr_t 	draw_commands_offset;

	Engine::DrawCommand command;
	...
	for (const auto &[mesh, instance_count] : m_geometry_counter) {
		...
		glNamedBufferSubData (
			...
			sizeof (Engine::DrawCommand),
			(void *) &command
		);

		draw_commands_offset += sizeof (Engine::DrawCommand);
	}
}

void Engine::Batch::syncInstancesSSBO () const {
	intptr_t offset;
	...
	for (const auto &obj : m_loaded_objects) {
		...
		glNamedBufferSubData (
			m_instance_ssbo,
			offset,
			sizeof (Engine::InstanceData),
			(void*) &obj->m_data
		);
		offset += sizeof (Engine::InstanceData);
	}
}