...
namespace TLOT
{
	...
	public:
		uint32_t GetIndex (std::vector<ResourceHandle> textures)
		{
			...
			ID_64 hash = GenerateOrderedSetHash (textures); // Generating a ordered hash from the handle allows to reuse space when instances need the same set of textures
		...
		ID_64 GetHash (std::vector<ResourceHandle> textures)
		{
			return GenerateOrderedSetHash (textures);
		}

	private:
		std::map<ID_64, size_t> m_texturesIndices;
